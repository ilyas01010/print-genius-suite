...
const ApiIntegrations = ({ integrationStatuses = [], onStatusChange }: ApiIntegrationsProps) => {
  ...
  const platforms: PlatformInfo[] = [
    ...
    },
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" height="24" width="24" className="text-blue-600 dark:text-blue-400">
          <path fill="currentColor" d="M24,6C14.061,6,6,14.061,6,24s8.061,18,18,18s18-8.061,18-18S33.939,6,24,6z M24,13 c5.514,0,10,4.486,10,10c0,5.514-4.486,10-10,10c-5.514,0-10-4.486-10-10C14,17.486,18.486,13,24,13z"/>
        </svg>
      )
    }
  ];
  ...
  // Connect handler for integration modal
  const handleConnect = async (platformId: string, credentials: any) => {
    // In a real application, this would make an API call to authenticate with the platform
    console.log(`Connecting to ${platformId} with:`, credentials);
    ...
    if (success) {
      ...
      if (onStatusChange) {
        onStatusChange(platformId, "connected");
      }
    }
  ...
  const handlePlatformClick = (id: string) => {
    ...
    if (status === "connected") {
      ...
      if (window.confirm(`Are you sure you want to disconnect ${platforms.find(p => p.id === id)?.name}?`)) {
        if (onStatusChange) {
          onStatusChange(id, "disconnected");
        }
        toast({
          title: "Platform Disconnected",
          description: `${platforms.find(p => p.id === id)?.name} has been disconnected.`,
        });
      }
    } else {
      // If not connected, open integration modal
      handleOpenModal(id);
    }
  };
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Connect external services and manage your API key
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-medium mb-3">Connect Platforms</h3>
            {platforms.map(platform => (
              <IntegrationItem
                ...
                initialStatus={getIntegrationStatus(platform.id)}
                onStatusChange={handleStatusChange(platform.id)}
                onConnect={() => {
                  handleOpenModal(platform.id);
                  return Promise.resolve(false); // Don't complete connection here
                }}
                icon={platform.icon}
              />
            ))}
      ...
      {activeModal && (
        <IntegrationModal
          ...
          platform={platforms.find(p => p.id === activeModal) || platforms[0]}
          onConnect={(credentials) => handleConnect(activeModal, credentials)}
        />
      )}
    </>
  );
