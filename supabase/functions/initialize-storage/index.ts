...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Creating designs bucket if it does not exist...')
    ...
    if (!designsBucketExists) {
      ...
      // Set up bucket policies to allow authenticated users to upload and download
      const { error: policiesError } = await supabaseAdmin.storage.from('designs').createPolicy('authenticated_upload', {
